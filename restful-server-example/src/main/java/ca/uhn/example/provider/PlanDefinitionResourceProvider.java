...
public class PlanDefinitionResourceProvider implements IResourceProvider {
	 ...
	 */
	private Map<Long, Deque<PlanDefinition>> myIdToPatientVersions = new HashMap<Long, Deque<PlanDefinition>>();
	 ...
	 */
	@Search()
	public List<PlanDefinition> findPatientsByName(@RequiredParam(name = PlanDefinition.SP_NAME) StringDt name) {
		LinkedList<PlanDefinition> retVal = new LinkedList<PlanDefinition>();

		/*
		 * Look for all patients matching the name
		 */
		for (Deque<PlanDefinition> nextPatientList : myIdToPatientVersions.values()) {
			PlanDefinition plan = nextPatientList.getLast();
	 ...
	 */
	@Read(version = true)
	public PlanDefinition readPatient(@IdParam IdDt theId) {
